...
The Chinese A-share market also provides a useful institutional setting for studying limits to arbitrage and sentiment-driven mispricing more generally. \citet{stambaugh2012short}, \citet{baker2007investor}, and \citet{shleifer1997limits} show that sentiment effects are largest where arbitrage is most costly, retail trading is most active, and short-selling is most restricted. The Chinese market satisfies all three conditions: only a small subset of stocks is borrowable for shorting, retail investors account for the majority of trading volume, and T+1 settlement further raises the cost of fast arbitrage \citep{liu2019size,jones2025retail,han2022chinese}. We therefore expect any sentiment-driven return predictability we document to be most pronounced in growth-oriented and small-capitalization indices, where retail participation is highest and arbitrage is most binding. The directional sign of the predictability we report---negative coefficients on lagged \textit{PhotoPes}, with the strongest effect on the ChiNext Composite---is interpreted through the overreaction lens of \citet{de1987further}: high-pessimism news triggers a short-horizon overreaction in retail-driven indices that subsequently corrects, with limits to arbitrage allowing the deviation to persist long enough to be measured at a multi-day horizon.

Following the multiple-testing standards advocated by \citet{harvey2016cross}, we pre-specify our primary hypothesis: $\textit{PhotoPes}_{t-3}$ predicts ChiNext Composite returns with a negative sign at conventional levels after Bonferroni adjustment across the five indices. The corresponding tests on the other four indices and on $\textit{TextPes}$ are treated as exploratory; we report them as evidence on cross-sectional heterogeneity rather than as additional independent confirmations.

Relative to the closest prior work, our contribution is more about geographic and methodological extension than about replacing existing visual-sentiment measures. Compared to \citet{obaid2022picture}, we extend the photo-pessimism approach from U.S. news photographs to Chinese news photographs and document a small-cap-concentrated reversal pattern that mirrors theirs. Compared to the recent \citet{gu2026giffluence} \emph{GIFfluence} working paper, which uses social-media animated GIFs to measure sentiment in U.S. equities, we use static news photographs from a financial-media homepage in a different market, providing an independent visual-sentiment data source. Compared to \citet{zhang2025multimodal}, who study multimodal sentiment on a Chinese sample using an Inception-v3 image backbone, we use a longer 2014--2026 sample window, a Vision Transformer image backbone explicitly anchored to \citet{obaid2022picture}, and Sina News headlines rather than China News Service text, generating an independent replication on a different newswire.
...
First, this paper utilizes the Vision Transformer model from deep learning to classify the sentiment of news images, addressing the technical challenges of large-scale image sentiment analysis, and constructs a photo-based pessimism indicator. Through the analysis of this indicator, the study finds that changes in news image sentiment have a significant effect on stock market return reversals, especially during periods of extreme sentiment, where the predictive power of image sentiment is more prominent. Compared to traditional text analysis methods, the image sentiment indicator can more intuitively and vividly capture changes in investor sentiment, demonstrating the unique advantages of image information in the sentiment quantification of financial markets.
...
Furthermore, this paper applies the sentiment indicators to construct a quantitative trading strategy. The strategy demonstrates strong risk-adjusted returns, outperforming its benchmark, though its factor-model alpha is positive but not statistically significant at conventional levels. An out-of-sample analysis using an expanding-window Ridge regression model (lags 3--5 of PhotoPes as predictors) confirms that sentiment predictability extends beyond the estimation sample: the ChiNext Composite achieves $R^2_{OOS} = 0.29\%$ significant at the 5\% level ($t = 2.02$), and the SZSE Composite and CSI 500 are significant at the 10\% level, providing evidence that the predictive relationship is not purely an in-sample artifact.
...
The most directly comparable contemporaneous study is \citet{zhang2025multimodal}, who construct multimodal sentiment indices on China News Service text and images using RoBERTa and an Inception-v3 backbone. Our paper differs from theirs along three axes that we view as complementary rather than competing: (i) sample length---our 2014--2026 sample (2{,}982 trading days) is roughly twice as long as theirs and includes the 2015 crash, the 2018 trade-tension drawdown, the 2020 COVID-19 shock, and the 2022 regulatory tightening; (ii) image backbone---we use a Vision Transformer to remain methodologically anchored to the most recent generation of computer-vision models, while \citet{zhang2025multimodal} use a CNN; and (iii) data source---we collect from Sina Financial News, an independent newswire, providing an out-of-sample replication of the multimodal-sentiment-China nexus on different underlying media. \citet{ren2025investor} similarly study image-based investor sentiment but apply few-shot learning to a U.S. NYT-image sample, leaving a clear gap for systematic ViT-based analysis on Chinese-language news media that this paper fills.
...
\begin{figure}[h!]
    \includegraphics[width=1\textwidth]{figures/vit_architecture.png}
...
%--------------------------------------------------------------------------
\subsection{Trading Strategy as Noise-Trader-with-Frictions Evidence}
This subsection translates the in-sample regression evidence into a portfolio backtest. We treat the resulting backtest not as a recommended trading strategy but as additional evidence on the noise-trader-with-frictions mechanism of \citet{stambaugh2012short}: if pessimism shocks really do generate predictable subsequent reversals in a retail-dominated, short-sale-restricted, T+1-settled market, then a long-only rule that systematically exploits high-pessimism days should generate a return spread relative to a passive benchmark, regardless of whether that spread survives transaction costs or factor adjustment in any specific implementation. The portfolio construction process is mainly divided into two stages: signal generation and position allocation. In the signal generation stage, this paper adopts a rule-based approach leveraging the predictive power of \textit{PhotoPes} and \textit{TextPes} sentiment indicators.
...
The empirical results show that the sentiment-driven strategy outperformed the benchmark during the sample period. As shown in Table~\ref{tab:performance_comparison}, the strategy achieved an annualized return of 9.03\% with an annualized volatility of 24.86\%, resulting in an excess-return Sharpe ratio of 0.318. In contrast, the CSI 500 Index recorded an annualized return of 6.23\% with an annualized volatility of 25.14\%, leading to an excess-return Sharpe ratio of 0.200. Sharpe ratios are computed on excess returns ($r_t - r_{f,t}$), where $r_{f,t}$ is the daily Chinese risk-free rate (one-year deposit rate, converted to daily) bundled with the CUFE-distributed Chinese Fama--French three-factor file, for consistency with the FF3 alpha calculation in Table~\ref{tab:factor_regression}. The strategy demonstrated modestly superior risk control; its maximum drawdown was $-62.22$\%, compared to the benchmark's maximum drawdown of $-71.11$\%. We interpret these numbers as evidence on the existence of statistical predictability in a retail-dominated market with binding short-sale constraints and T+1 settlement, consistent with the noise-trader-with-frictions framework \citep{stambaugh2012short,han2022chinese}, rather than as a transaction-cost-net trading recommendation. The reported Sharpe ratio includes neither bid--ask spreads, stamp duties, nor market-impact costs, which historical estimates put at roughly 30--40~bps round-trip in the CSI 500 universe.
...
Although the magnitudes of $R^2_{OOS}$ are small in absolute terms, \citet{campbell2008predicting} demonstrate that even modest positive values at the daily frequency can translate into meaningful economic gains for a mean-variance investor; \citet{rapach2010out} report monthly OOS $R^2$ values of 0.5--1.0\% as economically meaningful for the U.S. equity premium, against which our daily 0.29\% on the ChiNext is of comparable order of magnitude after accounting for frequency. The pattern of OOS predictability---strongest for growth-oriented, smaller-capitalization indices (ChiNext, SZSE Composite, CSI 500) with higher retail investor participation, and weakest for the large-cap CSI 300 and SSE Composite---is consistent with behavioral finance theory that sentiment effects are most pronounced for stocks that are harder to value and more difficult to arbitrage \citep{baker2007investor,stambaugh2012short}. The rule-based sentiment strategy described in the portfolio construction subsection operates on pre-identified signal thresholds rather than estimated regression coefficients, and its performance should be interpreted independently from the OOS regression results.
...
To examine the temporal stability of the OOS predictability, we partition the 1,467 OOS predictions into three equal-length sub-windows of 489 trading days each. Table~\ref{tab:oos_subsample} reports $R^2_{OOS}$ and the Clark--West MSPE-adjusted $t$-statistic in each window. The pattern is informative. Window~1 (March 2020 to March 2022) covers the post-COVID recovery and 2021 small-cap rally; in this period the model attains $R^2_{OOS}$ between 0.71\% and 0.87\% across all five indices, with significance at the 5\% level or better. Window~2 (March 2022 to April 2024) coincides with a prolonged drawdown, regulatory tightening, and a structural shift in retail-investor activity; predictability turns slightly negative for all indices, indicating that the historical mean was the better forecast in that regime. Window~3 (April 2024 to April 2026) shows a partial recovery, with the ChiNext attaining $R^2_{OOS} = 0.54\%$ ($t = 1.59$, $p = 0.056$). The full-sample OOS $R^2$ in Table~\ref{tab:oos_results} therefore averages over heterogeneous regimes; the headline ChiNext result is driven primarily by Windows~1 and~3, not by uniform performance across time. We view this pattern as consistent with sentiment-driven predictability being concentrated in periods of elevated retail activity and volatility, and we report it transparently rather than averaging it away.
...
\begin{table}[h!]
    ...
    \midrule
    $R^2_{OOS}$ (\%)        & 0.87$^{***}$ & 0.85$^{***}$ & 0.75$^{**}$  & 0.76$^{**}$  & 0.71$^{**}$  \\
    MSPE-adj $t$            & 2.56         & 2.57         & 2.06         & 2.08         & 2.07         \\
    \midrule
    \multicolumn{6}{l}{\textit{Window 2: 2022-03-28 to 2024-04-01, $N=489$}} \\
    \midrule
    $R^2_{OOS}$ (\%)        & $-$1.08      & $-$0.74      & $-$0.63      & $-$0.94      & $-$0.61      \\
    MSPE-adj $t$            & $-$1.46      & $-$0.83      & $-$0.42      & $-$0.55      & $-$0.44      \\
    \midrule
    \multicolumn{6}{l}{\textit{Window 3: 2024-04-02 to 2026-04-10, $N=489$}} \\
    \midrule
    $R^2_{OOS}$ (\%)        & $-$0.03      & $-$0.10      & 0.06         & 0.54$^{*}$   & 0.10         \\
    MSPE-adj $t$            & 0.37         & 0.21         & 0.63         & 1.59         & 0.71         \\
    \bottomrule
    \end{tabular}

    \vspace{0.2cm}
    \footnotesize
    \textit{Note:} Same expanding-window Ridge model as Table~\ref{tab:oos_results}; the 1{,}467 OOS predictions are partitioned into three equal-length date-ordered sub-windows. Significance markers ***, **, * follow the one-sided Clark--West MSPE-adjusted test at the 1\%, 5\%, and 10\% levels.
\end{table}

\subsubsection{Economic Significance: Certainty-Equivalent Gain}
Following \citet{campbell2008predicting} and \citet{rapach2010out}, we translate the OOS predictive accuracy into the certainty-equivalent (CER) gain that a mean-variance investor would realize by switching from the historical-mean forecast to the sentiment-conditional forecast. At each OOS date $t$, the investor allocates portfolio weight $w_t = \hat\mu_t / (\gamma \hat\sigma_t^2)$ to the index and the residual to a zero-return outside option, with weights clipped to $[0, 1.5]$ to rule out short-selling and large leverage; $\hat\sigma_t^2$ is the expanding-window historical variance, and $\hat\mu_t$ is either the model forecast or the historical mean. The CER is then $\bar r_p - \frac{1}{2}\gamma \cdot \mathrm{Var}(r_p)$, and the CER gain is the difference between the model and benchmark CERs, annualized by 252 and reported in percentage points. Table~\ref{tab:oos_cer} shows the gain at $\gamma = 3$ and $\gamma = 5$. The ChiNext Composite delivers a CER gain of $+10.17$\%~p.a.\ at $\gamma=3$ and $+5.55$\%~p.a.\ at $\gamma=5$, by far the largest among the five indices. The other indices produce CER gains of $+5.6$\% to $+6.7$\% at $\gamma=3$ and $+2.7$\% to $+3.7$\% at $\gamma=5$. These figures should be interpreted as upper-bound economic magnitudes that ignore transaction costs, stamp tax, and market-impact slippage; even so, the ranking across indices---ChiNext largest, large-caps smallest---tracks the in-sample regression evidence and reinforces the noise-trader-with-frictions interpretation.
...
\begin{table}[h!]
    ...
    \midrule
    $\gamma = 3$ (\% p.a.) & $+5.58$ & $+5.66$ & $+6.74$ & $+10.17$ & $+6.75$ \\
    $\gamma = 5$ (\% p.a.) & $+2.68$ & $+3.12$ & $+3.66$ & $+5.55$  & $+3.74$ \\
    \bottomrule
    \end{tabular}

    \vspace{0.2cm}
    \footnotesize
    \textit{Note:} CER gain is the annualized difference between the certainty equivalent of a mean-variance investor using the PhotoPes-conditional forecast and the certainty equivalent of the same investor using the expanding-window historical-mean forecast. Portfolio weights are $w_t = \hat\mu_t / (\gamma \hat\sigma_t^2)$ clipped to $[0, 1.5]$; $\hat\sigma_t^2$ is the expanding-window historical variance. The OOS period is 2020-03 to 2026-04 with 1{,}467 observations. The clip allows up to 50\% leverage, which is not generally accessible to retail investors in the Chinese A-share market; CER gains under a no-leverage constraint $w_t \in [0, 1]$ are smaller, and bid--ask spreads, stamp duty, and market-impact costs (estimated at 30--40~bps round-trip) further attenuate the figures. The numbers should therefore be read as upper-bound magnitudes consistent with the predictability documented in Table~\ref{tab:oos_results}, not as feasible after-cost returns.
\end{table}
...
Because we run $\textit{PhotoPes}_{t-3}$ tests across five indices, the headline 1\% $p$-value on the ChiNext requires a multiple-testing adjustment to maintain a family-wise Type-I error rate below conventional levels. Following \citet{harvey2016cross}, we treat the ChiNext result as our pre-specified primary hypothesis and apply a Bonferroni correction with $m=5$ to the secondary indices. The Bonferroni-adjusted threshold for 5\% family-wise significance becomes $p < 0.01$. Under this correction, the ChiNext $\textit{PhotoPes}_{t-3}$ result (raw $p < 0.01$, $|t|=2.81$) survives at the 5\% family-wise level, while the SZSE Composite and CSI 500 results (raw $p \approx 0.06$--$0.09$) become exploratory rather than confirmatory. We pre-specify the $t-3$ lag based on the lag at which prior visual-sentiment work \citep{obaid2022picture} reports the strongest reversal effect, so the relevant family is the five indices at one lag, not all $5\times5 = 25$ index--lag combinations. A more conservative correction with $m=25$ would set the family-wise 5\% threshold at $p<0.002$, which the ChiNext result does not meet ($|t|=2.81$, raw $p\approx 0.005$); under that more aggressive correction, all secondary lags and indices remain exploratory and the ChiNext effect should be read as suggestive rather than confirmatory. We report both interpretations transparently and prefer the $m=5$ reading because the lag was selected ex ante.
...
\subsection{Visual Domain Shift: ViT Stability Across Image-Quality Strata and Years}
\label{sec:vit_robustness}
A natural concern with our PhotoPes indicator is that the ViT classifier is trained on the social-photo distribution of \citet{you2015robust} and applied out-of-domain to Sina financial-news photos. To evaluate the consequences of this distribution shift, we construct a \textbf{stratified evaluation panel of 6{,}107 Sina images} drawn approximately uniformly across calendar years (about 500 images per year for 2015--2026, plus 107 from 2014). Each image in this panel has been independently scored on a [0,1] quality scale by \textbf{three frontier multimodal large language models} (GPT-4o-mini, Claude Sonnet 4.6, and Gemini 3 Pro Preview) using identical prompts; majority-vote labels classify each image as \texttt{quality\_pass} (clear, interpretable financial photo), \texttt{quality\_review\_needed}, or \texttt{quality\_fail\_candidate} (text-heavy infographic, blurry, or otherwise unsuitable). We then re-evaluate our production ViT on every image and compare the distribution of pessimism scores conditional on stratum and year.

Table~\ref{tab:vit_quality_robustness} reports the results. ViT predicts a higher mean pessimism on \texttt{quality\_fail} images (0.260) than on \texttt{quality\_pass} images (0.158); the difference is statistically significant (Welch's $t = -12.07$, $p < 10^{-32}$; Kolmogorov--Smirnov $D = 0.134$, $p < 10^{-13}$), with a small-to-moderate standardised effect size of Cohen's $d \approx 0.34$. On the \emph{frontier-LLM consensus subset}---the 949 images on which all three models agree on the same quality label---the gap between \texttt{quality\_pass} (0.185) and \texttt{quality\_fail} (0.408) widens to 0.223, roughly $2.2$ times the analogous gap of 0.102 in the full panel. We read this pattern as evidence that ViT responds to systematic visual content rather than to noise: when frontier general-purpose models converge on the verdict that an image is unsuitable for sentiment inference (text-heavy infographic, blurry, or otherwise degraded), our specialised ViT also produces a noticeably different score distribution. The share of \emph{decisive} predictions (output probability $\hat{\pi}<0.2$ or $\hat{\pi}>0.8$) is similar across quality strata in the full panel (82\%--88\%) and in the consensus subset (77\%--83\%), indicating that ViT does not collapse onto an uninformative neutral score on out-of-distribution images.

Panel C of Table~\ref{tab:vit_quality_robustness} reports temporal stability: across the 13 yearly strata of the \texttt{quality\_pass} subset, the cross-year standard deviation of the mean ViT pessimism score is 0.029, with year-level means clustered in the range $[0.094, 0.206]$. This magnitude is of the same order as the per-year sampling noise implied by the within-year SD ($\approx 0.21$) and per-year sample sizes (8--169), so we read the result as consistent with no meaningful temporal drift in ViT predictions over the 2014--2026 sample.

\begin{table}[h!]
    \centering
    \caption{ViT Pessimism Distribution by Image-Quality Stratum and Year (2014--2026)}
    \label{tab:vit_quality_robustness}
    \setlength{\tabcolsep}{6pt}
    \small
    \begin{tabular}{lcccc}
    \toprule
    Stratum & N & Mean & SD & \% decisive \\
    \midrule
    \multicolumn{5}{l}{\textit{Panel A: Full annotation panel ($N = 6{,}102$ successful inferences)}} \\
    \midrule
    \texttt{quality\_pass\_auto\_accept}   & 1{,}253 & 0.158 & 0.213 & 83.6\% \\
    \texttt{quality\_review\_needed}       & 1{,}812 & 0.216 & 0.308 & 87.9\% \\
    \texttt{quality\_fail\_candidate}      & 3{,}037 & 0.260 & 0.327 & 82.8\% \\
    \midrule
    \multicolumn{5}{l}{\textit{Panel B: Frontier-LLM consensus subset (3-of-3 agreement, $N = 949$)}} \\
    \midrule
    \texttt{quality\_pass\_auto\_accept}   & \phantom{0,}249 & 0.185 & 0.216 & 77.1\% \\
    \texttt{quality\_review\_needed}       & \phantom{0,}295 & 0.268 & 0.330 & 83.4\% \\
    \texttt{quality\_fail\_candidate}      & \phantom{0,}405 & 0.408 & 0.379 & 76.8\% \\
    \midrule
    \multicolumn{5}{l}{\textit{Panel C: Year stability on \texttt{quality\_pass} subset (year-level summary)}} \\
    \midrule
    Cross-year mean range            & \multicolumn{4}{l}{$[0.094,\ 0.206]$ across 13 calendar years} \\
    Cross-year SD of yearly means    & \multicolumn{4}{l}{$0.029$} \\
    Welch's $t$ (pass vs.~fail)      & \multicolumn{4}{l}{$-12.07$ \ ($p = 6.5 \times 10^{-33}$)} \\
    KS distance (pass vs.~fail)      & \multicolumn{4}{l}{$0.134$ \ ($p = 2.1 \times 10^{-14}$)} \\
    \bottomrule
    \end{tabular}

    \vspace{0.2cm}
    \footnotesize
    \textit{Note:} The annotation panel comprises 6{,}107 Sina financial-news images drawn approximately uniformly across calendar years 2014--2026; each image was scored on a [0,1] quality scale by GPT-4o-mini, Claude Sonnet 4.6, and Gemini 3 Pro Preview using identical prompts, with majority-vote labels assigned to each image. ``\% decisive'' is the share of ViT predictions with pessimism probability outside $[0.2, 0.8]$. Panel B restricts to images on which all three frontier models agree. Panel C summarises the temporal stability across the 13 calendar years on the \texttt{quality\_pass\_auto\_accept} subset and reports two-sample tests of the difference between the \texttt{quality\_pass} and \texttt{quality\_fail} pessimism distributions.
\end{table}
...
The research findings show that photo sentiment has a significant impact on market returns over the 2014--2026 sample period. Specifically, \textit{PhotoPes}$_{t-3}$ demonstrates consistent negative predictive power, with particularly strong effects in growth-oriented markets such as ChiNext (significant at the 1\% level) and smaller-cap indices. This finding is directionally consistent with the overreaction hypothesis in behavioral finance and persists in the joint regression specification controlling for text sentiment.

Text sentiment ($\text{TextPes}_{t-3}$) shows complementary predictive patterns, particularly for the SZSE Composite (SZCOMP) and ChiNext indices, though with somewhat weaker statistical significance in this extended sample. The conditional analysis further shows that these predictive effects are concentrated in high-volatility market states: during high-volatility periods, both $\text{PhotoPes}_{t-3}$ and $\text{TextPes}_{t-3}$ are significant predictors of returns on the SZSE Composite, ChiNext, and CSI 500, whereas neither indicator exhibits meaningful predictive power during low-volatility periods.

In terms of investment applications, this paper constructs a rule-based trading exercise that converts the in-sample predictability into an executable long-only signal. The exercise produces a higher excess-return Sharpe ratio than the CSI 500 benchmark (0.318 versus 0.200) and a smaller maximum drawdown, but the corresponding three-factor alpha relative to the CUFE-distributed Chinese Fama--French \citeyearpar{fama1993common} factors is positive yet statistically insignificant. We interpret this as further evidence consistent with the noise-trader-with-frictions framework of \citet{stambaugh2012short} in a market where short-selling is constrained, rather than as a recommendation of a transaction-cost-net trading strategy; the reported figures do not include bid--ask spreads, stamp tax, or market-impact costs.
