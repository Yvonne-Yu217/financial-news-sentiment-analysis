...
Furthermore, this paper applies the sentiment indicators to construct a quantitative trading strategy. The strategy demonstrates strong risk-adjusted returns, outperforming its benchmark and generating significant alpha. An out-of-sample analysis using an expanding-window Ridge regression model (lags 3--5 of PhotoPes as predictors) confirms that sentiment predictability extends beyond the estimation sample: the ChiNext Composite achieves $R^2_{OOS} = 0.29\%$ significant at the 5\% level ($t = 2.02$), and the SZSE Composite and CSI 500 are significant at the 10\% level, providing evidence that the predictive relationship is not purely an in-sample artifact.
...
Text sentiment ($\text{TextPes}_{t-3}$) shows complementary predictive patterns, particularly for the SZSE Composite (SZCOMP) and ChiNext indices, though with somewhat weaker statistical significance in this extended sample. The conditional analysis reveals an interesting interaction pattern: during normal sentiment periods, photo sentiment is the dominant predictor at the $t-3$ and $t-4$ lags, while extreme sentiment periods are characterized by text sentiment predictability at the $t-2$ lag.
