...
Furthermore, this paper applies the sentiment indicators to construct a quantitative trading strategy. The strategy delivers a higher gross-of-cost Sharpe ratio than its benchmark, though its factor-model alpha is positive but not statistically significant at conventional levels and the gross Sharpe gap is not statistically distinguishable from zero under stationary block bootstrap. An out-of-sample analysis using an expanding-window OLS forecast (lags 3--5 of PhotoPes as predictors) confirms that the predictive relationship survives in true out-of-sample data: the ChiNext Composite attains $R^2_{OOS} = 0.267\%$, significant at the 5\% level ($t = 2.12$), and the SHCOMP, SZSE Composite, and CSI 500 are marginally significant at the 10\% level. We use OLS rather than Ridge regression as the primary specification because, with three predictors and 1{,}512 training observations, the dimension ratio is well below the threshold at which regularization improves out-of-sample performance \citep{campbell2008predicting}; Ridge results are reported as a robustness check.
...
In terms of investment applications, this paper constructs a rule-based trading exercise that converts the in-sample predictability into an executable long-only signal. The exercise produces a higher gross-of-cost excess-return Sharpe ratio than the CSI 500 benchmark (0.318 versus 0.200) and a smaller maximum drawdown, but two referee-mandated robustness exercises sharply qualify these figures: (i) a stationary block-bootstrap shows that the gross Sharpe gap is not statistically distinguishable from zero (95\% percentile CI $[-0.75, +0.97]$, one-sided $p=0.39$), and (ii) the strategy Sharpe falls below the benchmark at approximately 17 basis points of round-trip transaction cost, well within the realistic range for Chinese A-share retail trading. The corresponding three-factor alpha relative to the CUFE-distributed Chinese Fama--French \citeyearpar{fama1993common} factors is positive yet statistically insignificant. We retain the trading exercise as evidence on the existence of statistical predictability in a retail-dominated market with binding short-sale constraints and T+1 settlement, consistent with the noise-trader-with-frictions framework of \citet{stambaugh2012short}, rather than as a recommendation of a tradable strategy. An out-of-sample analysis using an expanding-window OLS forecast of PhotoPes lags 3--5 confirms the in-sample finding qualitatively: the ChiNext attains $R^2_{OOS}=0.267\%$ ($t=2.12$, $p=0.017$) under one-sided MSPE-adjusted inference, with the SHCOMP, SZSE Composite, and CSI 500 marginally significant at the 10\% level.
