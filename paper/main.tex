...
%--------------------------------------------------------------------------
\begin{abstract}
Using a 12-year panel of 245{,}000+ Sina Finance news photos and articles paired with daily returns on five Chinese A-share indices (2014--2026), we document a state-dependent, cross-sectional pattern of multimodal-sentiment predictability that contrasts with the substitute relationship between visual and textual sentiment reported for the U.S.\ market by \citet{obaid2022picture}. In high-volatility states, image-based pessimism (\textit{PhotoPes}, ViT-extracted) and text-based pessimism (\textit{TextPes}, RoBERTa-extracted) carry incremental and largely independent predictive content for next-three-day index returns. The strength of each signal is approximately ordered along three independent retail-density proxies (turnover, inverse free-float capitalization, and retail-account share, internally consistent at high Spearman correlation): the retail-driven ChiNext Composite is on top and the institution-dominated CSI 300 / SHCOMP at the bottom across all three. Pooled cluster-by-date inference that accounts for cross-index residual correlation places the headline at the 10\% level (full sample and high-volatility subsample); per-index decomposition shows the strongest contribution coming from the ChiNext, with the relationship most negative in 2014--2017, near zero in 2018--2023, and partially recovering in 2024--2026. An expanding-window OOS forecast under strictly causal rolling-window standardization confirms the in-sample pattern on the ChiNext (OLS $R^2_{OOS} = 0.29\%$; CV-Ridge $R^2_{OOS} = 0.015\%$). The same construction applied as a long/short trading rule produces no statistically distinguishable Sharpe gap and is fully eroded by realistic Chinese A-share frictions ($\sim$17~bps round-trip). We position the result as evidence on a behavioral channel concentrated in retail-driven indices, not as a tradable claim.
\end{abstract}
\JEL{G12, G14, G41}
...
(iii) we anchor the cross-sectional pattern to a measurable consistency-of-rank finding: the ordering of effect strength across the five indices is \emph{approximately} aligned with three independent retail-density proxies (the proxies themselves agree at Spearman $\rho \in [0.90, 1.00]$; the ChiNext is on top and the CSI 300 / SHCOMP are at the bottom across all three, with two off-diagonal swaps in the middle three indices), and the headline ChiNext effect is robust to Amihud illiquidity, news-volume, and news-activity placebo controls. We acknowledge that with only five cross-sectional observations the Spearman $\rho = 0.80$ between the coefficient ranking and any one proxy has two-sided $p \approx 0.10$ and is best read as a suggestive triple-source rank-convergence finding rather than a quasi-experimentally identified mechanism; a stock-level interaction extension is left for future work.
...
\paragraph{Multiple-testing.} Following \citet{harvey2016cross}, we report Bonferroni-adjusted family-wise thresholds at three plausible family sizes ($m \in \{5, 25, 50\}$) as a transparent menu rather than designating any one as primary. Under the small-family reading ($m=5$, lag borrowed from \citet{obaid2022picture}) the headline ChiNext result survives at 5\%; under the more conservative $m=25$ or $m=50$ readings it does not. Once the family-wise pooled cluster-by-date estimate is the relevant inference object (\S\ref{sec:pooled_cluster}), the Bonferroni accounting becomes moot for inferential purposes: the pooled estimate IS the family-wise estimate by construction.
...
Visual stimuli elicit emotional responses that are faster and more durable than purely textual stimuli \citep{powell2015visual}. Building on this evidence, the \citet{baker2007investor} corollary --- that sentiment effects concentrate in hard-to-arbitrage / retail-driven assets --- suggests that visual sentiment, to the extent that it is processed automatically rather than deliberatively, should propagate most strongly into prices in market segments where retail investors dominate and arbitrage is constrained. Despite this prediction, the application of visual-sentiment quantification in finance remains an emerging field; early research focused mainly on the informational content of charts and pictures in corporate annual reports, with limited treatment of news photographs.
...
Two clear gaps emerge from the literature surveyed above. First, sentiment quantification remains predominantly unimodal, with text analysis dominant and visual-information analysis comparatively underdeveloped; \citet{obaid2022picture} began to fill this gap on U.S.\ data, but the visual-sentiment mechanism in markets with different microstructure and investor composition has not been systematically examined. Second, although Chinese-market sentiment research has progressed, methods have largely borrowed from the U.S.\ literature without exploiting the cross-sectional dispersion in retail-density and limits to arbitrage that distinguishes Chinese A-share index families. The empirical contribution of the present paper, summarized in the introduction, is positioned in the intersection of these two gaps; we do not restate the contribution list here.
...
\begin{table}[h!]
    ...
    \vspace{0.2cm}
    \footnotesize
    \textit{Note:} The sample period is from 2014 to 2026. The returns of the major Chinese stock market indices are calculated as log daily returns. The full sample contains 2{,}982 trading days; regression analyses use $N=2{,}978$ after accounting for the five-period lag structure. Sentiment indicators are constructed from all news days in the sample. The sample correlation between $\textit{PhotoPes}_t$ and $\textit{TextPes}_t$ is approximately $\rho \approx 0.40$ (computed from the first-stage $R^2 = 0.16$ in the Tetlock-style orthogonalization in \S\ref{sec:conditional_analysis}); the two indicators capture distinguishable but partially related dimensions of investor sentiment.
\end{table}
...
This state-dependent pattern is consistent with the \citet{baker2007investor} prediction that sentiment effects are strongest when arbitrage is most costly, precisely the high-volatility states identified here. The simultaneous significance of both $\textit{PhotoPes}_{t-3}$ and $\textit{TextPes}_{t-3}$ during turbulent periods is consistent with image and text sentiment jointly capturing investor-overreaction dynamics, with the predictive content materializing three trading days after the initial media signal.
...
\paragraph{Retail-density alignment of effect strength.} The cross-sectional ranking of the high-volatility \textit{PhotoPes}$_{t-3}$ coefficient magnitude follows a clear ordering: ChiNext ($-$0.0033) $>$ CSI 500 ($-$0.0022) $\approx$ SZCOMP ($-$0.0021) $>$ CSI 300 ($-$0.0016) $\approx$ SHCOMP ($-$0.0014). We verify that this ranking is \emph{approximately} aligned with each of three independent retail-density measurements --- annualized turnover (CSI Index Co.\ data: ChiNext $\approx 6.30$, SZSE $\approx 4.85$, CSI 500 $\approx 3.24$, SHCOMP $\approx 1.27$, CSI 300 $\approx 1.08$), inverse free-float market capitalization (per-stock free-float in 2024-end snapshots from SSE/SZSE/CSI Index Co.\ statistics), and retail-account share \citep{liu2019size,jones2025retail}. The three retail-density proxies themselves agree internally with Spearman rank correlations in $[0.90, 1.00]$; the Spearman correlation between the PhotoPes coefficient ranking and any one proxy ranking is $0.80$ (two-sided $p \approx 0.10$ at $N=5$), with the top (ChiNext) and bottom (CSI 300) positions identical across all three proxies. The two off-diagonal swaps (CSI 500 vs.\ SZCOMP and SHCOMP vs.\ CSI 300) involve coefficient differences within rounding distance of one another, so the rank evidence reduces to two binary degrees of freedom (top/bottom). We read this as a \emph{triple-source rank-convergence finding} consistent with the \citet{baker2007investor} corollary that sentiment effects concentrate in hard-to-arbitrage / retail-driven assets --- not as a quasi-experimentally identified retail-density mechanism. The cross-sectional ordering is not a single-source artifact, but the available power for cross-sectional inference is limited; a stock-level interaction regression with hundreds of cross-sectional units, deferred to a follow-up paper, would tighten the identification.
...
%--------------------------------------------------------------------------
\subsection{In-Sample Falsification Check: A Long/Short/Neutral PhotoPes Trading Rule}
This subsection presents an in-sample illustrative falsification check against the strict-tradable interpretation of the regression results in \S\ref{sec:photopes_marginal}--\S\ref{sec:conditional_analysis}. We construct a long/short/neutral signal-thresholded rule from the \textit{PhotoPes} signal as faithfully as possible to the design of \citet{obaid2022picture} for the U.S.\ market, so that the comparison is structured as a cross-market replication. We caution at the outset that the strategy's $0.80/0.20$ image/text weighting is inherited from the in-sample high-volatility joint regression coefficient ratio in Table~\ref{tab:sentiment_significant_periods}, so the rule is being evaluated on the same in-sample period from which its weights are derived; the appropriate reading is therefore as an \emph{illustrative falsification check on a null}, not as a tradable claim or as a fully independent out-of-sample test. The construction details are deferred to the table note; we emphasize directly the inference results.
...
Figure~\ref{fig:investment_growth} illustrates the gross-of-cost cumulative returns of the sentiment-driven strategy and the CSI 500 Index, reported here purely as a visual companion to the formal bootstrap inference in the paragraph above and the transaction-cost grid in Table~\ref{tab:performance_comparison}. The figure shows that on a frictionless basis 1 RMB invested at the beginning of 2014 reaches approximately 2.02 RMB by April 2026 under the rule and 1.42 RMB under the CSI 500 benchmark, but this gross-of-cost gap is statistically indistinguishable from zero (bootstrap CI $[-0.75, +0.97]$) and is fully eroded at $\sim$17~bps round-trip (Table~\ref{tab:performance_comparison}). The figure should not be read as evidence of tradable outperformance; it is presented because the cross-market replication construction calls for it.

\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{figures/cumulative_returns_old.png}
    \caption{Gross-of-cost cumulative returns of the sentiment-driven strategy (signal-thresholded long/short/neutral rule, 5-day rebalance) and the CSI 500 Index, 2014--2026. Net-of-cost cumulative values at $\{10,20,30,40,50\}$ bps round-trip transaction costs are reported in Table~\ref{tab:performance_comparison}; the strategy Sharpe falls below the CSI 500 benchmark's $0.200$ at approximately 17 bps round-trip and the cumulative value falls below 1 RMB at 40 bps round-trip.}
...
We also examine sensitivity to the Newey--West HAC bandwidth. Table~\ref{tab:nw_bandwidth} re-runs the headline ChiNext PhotoPes$_{t-3}$ regression at a deterministic grid of bandwidths $\{5, 10, 15, 22\}$ together with the two standard data-driven plug-in selectors: \citet{andrews1991heteroskedasticity} and \citet{newey1994automatic}. The point estimate is unchanged across columns; the $t$-statistic varies smoothly across the bandwidth choices. The \citet{newey1994automatic} automatic selector picks 8 lags with $|t|=2.75$ ($p \approx 0.006$); we treat this as the default data-driven headline because it is rule-based without favoring the smallest bandwidth. The \citet{andrews1991heteroskedasticity} plug-in picks 1 lag and yields $|t|=2.93$, slightly stronger than NW(1994) but mechanically tied to a single-lag autoregressive null; we report it as a more aggressive alternative rather than as the default. Both data-driven selectors deliver 1\%-level rejection. The deterministic NW(5) value reported in the main tables ($|t|=2.83$) is matched to the right-hand-side lag depth as a deliberately conservative choice that absorbs autocorrelation up to the lag horizon present in the regression, and is essentially indistinguishable from the NW(1994) auto-select value. The headline ChiNext result is significant at the 1\% level for bandwidths up to 15 lags and at the 5\% level at 22 lags. With $T=2{,}977$ observations and the right-hand-side already absorbing five lags of returns and squared returns, the residual autocorrelation the HAC kernel needs to absorb is small, and the plug-in selectors correspondingly pick small bandwidths. The deterministic grid $\{5, 10, 15, 22\}$ confirms that the inferential conclusion is bandwidth-invariant.
...
The Bonferroni family-size choice is itself an unsettled methodological question \citep{harvey2016cross}, and we present the three thresholds as a transparent menu rather than mechanically defaulting to any one. The $t-3$ lag in our headline test is not a data-driven choice; it is borrowed forward from \citet{obaid2022picture}, who report the strongest reversal effect at a similar three-period horizon on weekly U.S.\ Wall Street Journal photographs. Under the standard ``research design borrowed from prior literature'' interpretation, the relevant family is the five indices at the single pre-specified lag, $m=5$ (per-test threshold $p<0.010$); the headline ChiNext result clears this threshold. Under the more conservative ``all five lags treated equally'' interpretation, the relevant family is $5 \times 5 = 25$ (per-test threshold $p<0.002$); the headline does not clear this threshold. Under the most conservative interpretation that additionally accommodates the parallel $\textit{TextPes}$ family ($m=50$, threshold $p<0.001$), no lag--index--indicator combination survives at the 5\% family-wise level. We acknowledge the lag transferability from Obaid's weekly U.S.\ design to the daily-frequency Chinese-A-share setting is not pinned down by that earlier paper; the small-family reading is therefore an interpretive choice the reader can disagree with, not a methodological default.

The Bonferroni accounting in Table~\ref{tab:multiple_testing} is moreover redundant once the family-wise pooled cluster-by-date estimate of \S\ref{sec:pooled_cluster} is the relevant inference object: that estimate (full-sample $t=-1.84$, $p=0.066$; high-vol $t=-1.86$, $p=0.063$; Driscoll--Kraay $t=-1.93$, $p=0.054$) IS the family-wise estimate by construction, so the family-size choice does not affect inference on the pooled coefficient. The robustness exercises in \S\ref{sec:conditional_analysis} (Tetlock orthogonalization, news-volume controls, sentiment-free placebos, Amihud liquidity controls) all retain ChiNext $\textit{PhotoPes}_{t-3}$ at the 5\% level under per-test inference, providing convergent support for the per-index ChiNext decomposition independent of the family-size choice.
...
This paper documents a cross-sectional pattern in Chinese A-share index returns that contrasts with the substitute relationship between visual and textual investor sentiment reported by \citet{obaid2022picture} for the U.S.\ market. In high-volatility states, both image-based pessimism (\textit{PhotoPes}, ViT-extracted from Sina Finance news photographs) and text-based pessimism (\textit{TextPes}, RoBERTa-extracted from headlines and articles) carry incremental predictive content for next-three-day index returns, with the strength of each modality scaling along an identifiable retail-density gradient. The cross-sectional ranking of \textit{PhotoPes} effect strength across the five indices (ChiNext $\approx -0.0033$, CSI 500 $\approx -0.0022$, SZCOMP $\approx -0.0021$, CSI 300 $\approx -0.0016$, SHCOMP $\approx -0.0014$) is approximately aligned with three independent retail-density measurements --- annualized turnover, inverse free-float market capitalization, and retail-account share --- which themselves agree internally at Spearman $\rho \in [0.90, 1.00]$ (ChiNext on top and CSI 300 on bottom across all three proxies). With $N=5$ cross-sectional units the Spearman $\rho = 0.80$ between coefficient and proxy rankings has two-sided $p \approx 0.10$, so we treat the result as a \emph{triple-source rank-convergence finding} consistent with the \citet{baker2007investor} corollary that sentiment effects concentrate in hard-to-arbitrage / retail-driven assets, rather than as a quasi-experimentally identified retail-density mechanism. To our knowledge it is the first systematic record of this state-dependent, cross-sectional pattern in the Chinese multimodal-sentiment literature, complementing the contemporaneous CSI 300 / chinanews.com sample of \citet{zhang2025multimodal}.
