...
(i) we record visual and textual investor sentiment in the Chinese A-share market on a 12-year sample (2014--2026, $\sim$245{,}000 articles and 572{,}000 photographs), extending the U.S.-focused multimodal-sentiment literature to a setting with materially different microstructure and investor composition;
...
A complementary line of evidence comes from event-window studies. \citet{hirshleifer2021fomc} use FOMC photographs and image-classification methods to extract a sentiment signal that predicts equity returns around scheduled monetary-policy announcements; their event-window design is methodologically distinct from the daily-aggregate, cross-index design pursued here, but the underlying premise --- that visual cues carry information beyond text --- is shared and reinforced.
...
\begin{table}[h!]
    ...
    \vspace{0.2cm}
    \footnotesize
    \textit{Note:} OLS regressions of ChiNext daily log returns on the headline right-hand-side specification (constant, L5(\textit{PhotoPes}), L5($R$), L5($R^2$), day-of-week dummies) plus a trailing cumulative-return control. The trailing cumulative return at $t-3$ over a $w$-day window is $\sum_{k=0}^{w-1} R_{t-3-k}$, computed using ChiNext daily log returns. Newey--West HAC standard errors with 5 lags. The reported $t$-statistics for trailing-window coefficients are the per-control NW(5) $t$ in each column; *** denotes 1\% significance on the headline coefficient. The contemporaneous correlation between $\textit{PhotoPes}_{t-3}$ and $R_{t-3}$ on the same sample is $+0.009$ ($n=2{,}980$), confirming that the headline $\textit{PhotoPes}_{t-3}$ predictor is not a noisy proxy for the lagged daily return.
\end{table}
...
The OOS results in Table \ref{tab:oos_results} confirm the in-sample finding that visual sentiment carries genuine predictive content for the most retail-driven Chinese index but provide weaker evidence elsewhere. The ChiNext Composite attains $R^2_{OOS} = 0.267\%$, statistically significant at the $5\%$ level based on the one-sided MSPE-adjusted test ($t = 2.12$, $p = 0.017$); this is the only clean positive predictability rejection. The SHCOMP ($R^2_{OOS} = 0.008\%$, $p=0.095$), SZSE Composite ($R^2_{OOS} = 0.013\%$, $p=0.072$), and CSI 500 ($R^2_{OOS}=-0.015\%$, $t=1.44$, $p=0.075$) carry one-sided MSPE-adjusted $p$-values below 0.10, but on three of these five indices the point estimate of $R^2_{OOS}$ is in fact non-positive --- a configuration that the Clark--West test can produce because it adjusts for the bias in the squared-error comparison induced by parameter estimation rather than testing whether the predictive model literally lowers MSE. We therefore read these three indices as marginal-rejection sampling-noise findings rather than as clean OOS-predictability rejections, and treat the ChiNext as the only index on which the in-sample association is reproduced cleanly out of sample. The CSI 300 has a negative point estimate and is not significant. The qualitative pattern---strongest predictability for the growth-oriented, retail-driven ChiNext, weaker for large-cap indices---mirrors the in-sample regression results in Table~\ref{tab:PhotoPes_regression_results} and is consistent with sentiment effects being most pronounced for stocks that are harder to value and more difficult to arbitrage \citep{baker2007investor,stambaugh2012short}.
