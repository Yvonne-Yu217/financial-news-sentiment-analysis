...
%--------------------------------------------------------------------------
\begin{abstract}
Using a 12-year sample of 245{,}000+ Sina Finance news articles paired with daily Chinese A-share index returns (2014--2026), we document a cross-sectional pattern that contrasts with the substitute relationship reported in U.S.\ data: in high-volatility states, both image-based and text-based news sentiment carry predictive content for future market returns, with the strength of each modality scaling along a retail-density gradient. Image-based pessimism (\textit{PhotoPes}, ViT-extracted) and text-based pessimism (\textit{TextPes}, RoBERTa-extracted) deliver negative coefficients on lagged returns concentrated in the growth-oriented, retail-driven ChiNext Composite (in-sample $|t|=2.81$, raw $p\approx 0.005$). The cross-sectional ranking of effect strength tracks three independent retail-density proxies (annualized turnover, inverse free-float market capitalization, retail-account share) with Spearman $\rho \in [0.90, 1.00]$, providing direct empirical anchoring of a retail-density-to-modality-sensitivity mechanism. An expanding-window OLS forecast yields out-of-sample $R^2_{OOS}=0.27\%$ on the ChiNext (one-sided MSPE-adjusted $t=2.12$, $p=0.017$), and an ex-ante regime-conditional version of the forecast (active only when trailing 60-day SHCOMP volatility exceeds the pre-OOS 60th percentile) raises the regime-active $R^2_{OOS}$ to $0.58\%$. The headline ChiNext effect is robust to controls for Amihud illiquidity, news-volume, and the Tetlock-style orthogonalization of \textit{PhotoPes} with respect to \textit{TextPes}, and is null on a sentiment-free image-count placebo. We additionally report a negative cross-market replication of \citet{obaid2022picture}: the same signal generates a long/short/neutral signal-thresholded rule whose Sharpe gap is not statistically distinguishable from zero under stationary block bootstrap and is fully eroded by realistic Chinese A-share frictions ($\sim$17 bps round-trip). The paper is positioned as evidence on the existence of statistical visual-sentiment predictability concentrated in retail-driven indices, not as a tradable claim.
\end{abstract}
\JEL{G12, G14, G41}
...
\paragraph{Identification strategy.} We employ a three-dimensional identification design. The \emph{index} dimension covers five Chinese A-share indices spanning the retail-density gradient: the institution-dominated CSI 300 and SHCOMP, the mid-cap SZCOMP and CSI 500, and the retail-driven ChiNext Composite. The \emph{state} dimension partitions trading days by 20-day rolling SHCOMP volatility into top-quartile (high-stress) versus the rest. The \emph{modality} dimension contrasts \textit{PhotoPes}, a daily aggregate built from a Vision Transformer trained on the \citet{you2015robust} Twitter sentiment corpus and applied to Sina Finance news photos, with \textit{TextPes}, built from the Erlangshen-Roberta-110M Chinese sentiment model applied to Sina Finance headlines and articles. Identification of the cross-sectional differential-sensitivity claim relies on consistent ordering of effect strength across the five indices and three retail-density proxies (turnover, inverse free-float market capitalization, retail account share). Identification of the state-dependence claim relies on the high-versus-low volatility regime contrast.

\paragraph{Contributions.} Our contribution is the intersection of three pieces; none alone would be a top-tier contribution but the combination is, to our knowledge, new:
(i) we record visual and textual investor sentiment in the Chinese A-share market on a 12-year sample (2014--2026, $\sim$245{,}000 articles and 572{,}000 photographs), filling a gap left by the U.S.-focused literature;
...
(iii) we anchor the cross-sectional pattern to a measurable mechanism by showing that the ranking of effect strength across the five indices aligns (Spearman $\rho \in [0.90, 1.00]$) with three independent retail-density proxies, and that the headline ChiNext effect is robust to Amihud illiquidity, news-volume, and news-activity placebo controls.

\paragraph{Negative cross-market replication.} An independent and informative finding deserves stand-alone mention: in U.S.\ data, \citet{obaid2022picture} report a statistically significant five-factor alpha for a long/short/neutral signal-thresholded rule built from their PhotoPes signal. In our Chinese A-share setting, the same construction produces a gross-of-cost Sharpe gap that is not statistically distinguishable from zero under stationary block bootstrap (95\% percentile CI $[-0.75, +0.97]$, one-sided $p=0.39$) and is fully eroded by approximately 17 basis points of round-trip transaction costs --- well within the realistic Chinese A-share retail trading-cost range. This is consistent with the present paper's framing: statistical predictability concentrated in retail-driven indices need not, and does not, translate into tradable returns once realistic frictions are imposed --- precisely the \citet{shleifer1997limits} ``limits of arbitrage'' prediction.
...
Among visual-sentiment work in finance, \citet{hirshleifer2021fomc} is the closest U.S.\ analog to our paper: they use FOMC photographs and image-classification methods to extract a sentiment signal that predicts equity returns in event windows. The combination of image classification + market-level sentiment + cross-sectional dispersion in retail share is, however, not the question that paper asks; we view our analysis as complementary, focused on the cross-modal and cross-index geometry under a Chinese institutional setting.
...
%--------------------------------------------------------------------------
\subsection{Negative Cross-Market Replication: A Long-Only PhotoPes Trading Rule}
...
The unconditional OOS results in Table~\ref{tab:oos_results} mix high-volatility and low-volatility days. If the underlying mechanism is, as the in-sample analysis suggests, concentrated in retail-driven, high-volatility regimes, then conditioning the forecast generation on an ex-ante regime indicator should sharpen the signal. To implement this without look-ahead bias, at each OOS date $t$ we compute the trailing 60-day realized volatility of SHCOMP using only data up to $t-1$. We then designate $t$ as ``regime-active'' if this trailing volatility exceeds the 60th percentile of the volatility distribution computed over the pre-OOS sample (i.e., before the OOS window opens), giving a fixed-in-advance threshold of 0.197 (annualized). The regime indicator is therefore strictly ex-ante. Table~\ref{tab:oos_regime} reports $R^2_{OOS}$ and the Clark--West $t$-statistic on the unconditional, regime-active, and regime-inactive subsamples for each of the five indices.

The regime conditioning sharpens the signal in the direction predicted by the M1 mechanism. On the ChiNext Composite, $R^2_{OOS}$ rises from 0.18\% unconditionally to 0.58\% on regime-active days (a 3.2$\times$ amplification of the magnitude); analogous improvements appear on all four other indices, with the regime-active $R^2_{OOS}$ ranging from 0.28\% (SHCOMP) to 0.58\% (SZCOMP, ChiNext) and the regime-inactive $R^2_{OOS}$ values close to zero or slightly negative for every index. The Clark--West $t$-statistics on regime-active days are weaker in some cases due to the smaller subsample size ($N=215$ regime-active days versus $N=1{,}411$ unconditional), but the magnitude pattern is unambiguous. Note that the unconditional ChiNext $R^2_{OOS}$ in this table (0.18\%) is computed on the OLS-on-raw-PhotoPes-lags spec (no rolling-window standardization correction) used for direct comparability with Table~\ref{tab:oos_results}'s 0.27\% headline; the rolling-window correction discussed in the appendix raises the unconditional figure to 0.29\% (see \S\ref{tab:oos_results} note). The regime-active 0.58\% and the regime-inactive near-zero figures are, however, robust to either standardization choice.
