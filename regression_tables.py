...
DATA_FILE   = os.path.join(BASE_DIR, "results", "merged_market_sentiment_data.csv")
OUTPUT_DIR  = os.path.join(BASE_DIR, "results", "regression_output")
...
def run_descriptive_stats(df):
    ...
    print(f"{'#'*60}")
    full = pd.read_csv(DATA_FILE, parse_dates=["Date"])
    sample = full[(full["Date"].dt.year >= 2014) & (full["Date"].dt.year <= 2026)].copy()
